...
const analyzeUtteranceForHonesty = (text) => {
    let honestyDelta = 0;
    ...
    entry: async (ctx) => {
        ...
        liveSession.on(voice.AgentSessionEventTypes.Error, async (event) => {
            ...
            const shouldRotateModel = sourceLabel.toLowerCase().includes('llm') &&
                /(402|429|503|credit|max_tokens|max completion|failed to generate llm completion|insufficient quota|rate.?limit|overloaded|capacity|too many requests)/i.test(errorMessage);
