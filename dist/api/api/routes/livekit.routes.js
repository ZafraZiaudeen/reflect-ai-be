...
import { clearMirrorAgentDispatch, createParticipantToken, dispatchMirrorAgent, } from '../../infrastructure/LiveKit/livekitService.js';
...
import { ensureLocalMirrorFallback, isLocalMirrorFallbackActive, isLocalMirrorFallbackEnabled, } from '../../infrastructure/Voice/localMirrorFallback.js';
export const livekitRouter = Router();
const STALE_DISPATCH_REFRESH_MS = 12_000;
const isLiveKitNotFoundError = (error) => typeof error === 'object' &&
    error !== null &&
    'status' in error &&
    error.status === 404;
livekitRouter.post('/token', asyncHandler(async (request, response) => {
    ...
    const shouldRefreshStaleDispatch = Boolean(session.liveKitDispatchId) && hasNoAgentTurnsYet && dispatchAgeMs > STALE_DISPATCH_REFRESH_MS;
