...
/* ------------------------------------------------------------------ */
const buildHomeworkContext = (couple) => {
...
/* ------------------------------------------------------------------ */
const analyzeUtterance = (room, text, speakerName) => {
...
/* ------------------------------------------------------------------ */
const connectDeepgramSTT = (room, conn) => {
...
/* ------------------------------------------------------------------ */
const processUserUtterance = async (room, text, speakerName, speakerRole) => {
    if (room.closed || !text.trim())
        return;
    ...
    try {
        ...
        if (analysis.intervention) {
            ...
            const interventionEvent = {
                id: randomUUID(),
                stage: analysis.detectedPatterns.includes('ai_attack') ? 'mirror'
                    : analysis.detectedPatterns.includes('denial') ? 'quote_evidence'
                        : 'interrupt',
...
/* ------------------------------------------------------------------ */
const synthesizeAndBroadcast = async (room, text, abortController) => {
    if (room.closed || room.interrupted || !text.trim())
        return;
    if (!env.DEEPGRAM_API_KEY)
        return;
...
/* ------------------------------------------------------------------ */
const saveTranscript = async (sessionId, segment) => {
    try {
...
const getOrCreateRoom = async (sessionId) => {
    ...
    const hasExistingTranscripts = sessionDoc.status === 'live' && sessionDoc.transcriptSegments?.length > 0;
    const room = {
        sessionId,
...
/* ------------------------------------------------------------------ */
const resolvePartner = (couple, userId) => {
...
/* ------------------------------------------------------------------ */
const handleVoiceConnection = async (ws, request) => {
    const url = new URL(request.url || '', `http://${request.headers.host || 'localhost'}`);
