...
import { RoomEvent, TrackKind, type Participant, type RemoteParticipant } from '@livekit/rtc-node';
import type { CoupleDocument } from '../../domain/Models/Couple.js';
import type { SessionDocument } from '../../domain/Models/Session.js';
import {
...
type RuntimeDependencies = {
  VoiceTelemetryApplication: typeof import('../../application/VoiceTelemetryApplication.js').VoiceTelemetryApplication;
  CoupleModel: typeof import('../../domain/Models/Couple.js').CoupleModel;
  SessionModel: typeof import('../../domain/Models/Session.js').SessionModel;
  connectDatabase: typeof import('../Database/connectDatabase.js').connectDatabase;
  createOpenRouterLlm: typeof import('../OpenRouter/openRouterService.js').createOpenRouterLlm;
};
...
const HOLDING_LINE_TEMPLATE = (name: string) =>
  `Welcome, ${name}. I'm Mirror, your session guide. Your partner hasn't joined the room yet — sit tight, and we'll begin together as soon as they're here.`;

let runtimeDependenciesPromise: Promise<RuntimeDependencies> | null = null;

const getRuntimeDependencies = (): Promise<RuntimeDependencies> => {
  runtimeDependenciesPromise ??= Promise.all([
    ...
    ([voiceTelemetryModule, coupleModule, sessionModule, databaseModule, openRouterModule]) => ({
...
const formatEvidenceTime = (timestamp: number): string => {
  const date = new Date(timestamp);
  return `${String(date.getHours()).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}`;
};

const trimEvidenceWindow = (entries: EvidenceEntry[]): void => {
...
const buildEvidenceSnippet = (entries: EvidenceEntry[]): string => {
...
const shouldInterveneFromTranscript = (entries: EvidenceEntry[]): boolean => {
  ...
  const repeatedAbsolutes = recentPartnerTurns.some((turn) =>
    /\b(always|never|nothing|everything)\b/i.test(turn.text),
  );
  const contemptLanguage = recentPartnerTurns.some((turn) =>
    /(ridiculous|crazy|pathetic|whatever|shut up|you always|you never|disgusting)/i.test(turn.text),
  );
...
const buildInterventionDecision = (args: {
  ...
  const hasDenial = /\b(you'?re wrong|that'?s not true|i didn'?t|i never|no i don'?t|not what happened)\b/i.test(
    normalizedTranscript,
  );
  const attacksTheAgent =
    /\b(bot|machine|ai|you don'?t know us|what do you know|you are not real|shut up mirror)\b/i.test(
      normalizedTranscript,
    );
  const refusesToEngage =
    /\b(i'?m done|we'?re done|whatever|leave me alone|not doing this|end this|stop talking)\b/i.test(
      normalizedTranscript,
    );

  if (refusesToEngage || (args.resistanceLevel >= 2 && (hasDenial || attacksTheAgent))) {
    const line =
      'If your goal is to dodge the work instead of face it, I will close this session. Choose honesty or we stop here.';
  ...
  if (attacksTheAgent) {
    const line =
      'Notice how fast you turned your anger toward me the moment I named the pattern. That is the defense I am talking about.';
...
export default defineAgent({
  entry: async (ctx) => {
    ...
    const { CoupleModel, SessionModel, VoiceTelemetryApplication, connectDatabase, createOpenRouterLlm } =
      runtimeDependencies;
    ...
    let sessionRecord: SessionDocument | null = null;
    ...
    try {
      const dbResult = await Promise.race([
        ...
        new Promise<{ session: null; couple: null }>((resolve) =>
          setTimeout(() => resolve({ session: null, couple: null }), DB_INIT_TIMEOUT_MS),
        ),
      ]);
    ...
    const modelAttempts = getModelAttemptOrder(
      metadata.selectedModel || sessionRecord?.selectedModel || sessionFallback.selectedModel,
    );
    ...
    const ensureRoomAudioOutputReady = () => {
      ...
      audioOutputStartTimer = setTimeout(() => {
        ...
        console.warn(
          `Mirror audio output did not confirm a subscription within ${AUDIO_OUTPUT_SUBSCRIPTION_GRACE_MS}ms for session ${sessionId}. Forcing output start.`,
        );
        latestParticipantAudioOutput.startedFuture.resolve();
      }, AUDIO_OUTPUT_SUBSCRIPTION_GRACE_MS);
    };

    const queuePrompt = (instructions: string, allowInterruptions = false) => {
      ensureRoomAudioOutputReady();
      const handle = liveSession.generateReply({
        instructions,
        allowInterruptions,
      });
      handle.waitForPlayout().catch((error) => {
        console.warn(`[mirror-agent] generateReply playout failed for session ${sessionId}:`, error);
      });
    };
    ...
    const speakLine = (text: string, allowInterruptions = false) => {
      ...
      handle.waitForPlayout().catch((error) => {
        console.warn(`[mirror-agent] speakLine playout failed for session ${sessionId}:`, error);
      });
    };

    const registerEvidence = (entry: EvidenceEntry) => {
      evidenceEntries.push(entry);
      trimEvidenceWindow(evidenceEntries);
    };

    const getCurrentHumanParticipant = (): HumanParticipant | null => {
    ...
    evaluateRoomState = () => {
      ...
      if (participantCount === 1 && !welcomedBothPartners) {
        ...
        if (!soloStartTimer) {
          // Start a solo session after a timeout regardless of whether partner B
          // exists. If partner B joins before the timer fires, evaluateRoomState
          // will cancel it and deliver the full welcome instead.
